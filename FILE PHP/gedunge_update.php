...
if (isset($_REQUEST['id_gedunge']) && isset($_REQUEST['text_edit'])) {
    // Sanitize and store the POST data in variables
    $id = mysqli_real_escape_string($conn, $_REQUEST['id_gedunge']);
    $text = mysqli_real_escape_string($conn, $_REQUEST['text_edit']);

    // SQL query to update data in the database
    $query = "UPDATE gedung_e SET text_edit = '$text' WHERE id_gedunge = '$id'";
